
import { Link } from 'react-router';
import Brand11 from '../../assets/images/brand/brand-1-1.png';
import Brand12 from '../../assets/images/brand/brand-1-2.png';
import Brand13 from '../../assets/images/brand/brand-1-3.png';
import Brand14 from '../../assets/images/brand/brand-1-4.png';
import Brand15 from '../../assets/images/brand/brand-1-5.png';
import Brand16 from '../../assets/images/brand/brand-1-6.png';
import { Swiper, SwiperSlide } from 'swiper/react';
import { Autoplay } from 'swiper/modules';
type BrandItem = {
    id: number;
    image: string;
    url: string;
};

const brandsData: BrandItem[] = [
    { id: 1, image: Brand11, url: "#" },
    { id: 2, image: Brand12, url: "#" },
    { id: 3, image: Brand13, url: "#" },
    { id: 4, image: Brand14, url: "#" },
    { id: 5, image: Brand15, url: "#" },
    { id: 6, image: Brand16, url: "#" },
    { id: 7, image: Brand11, url: "#" },
    { id: 8, image: Brand12, url: "#" },
    { id: 9, image: Brand13, url: "#" },
];

export default function BrandOne() {
    return (
        <section className="brand-one" >
            <div className="container" >
                <div className="brand-one__carousel owl-carousel owl-theme owl-loaded owl-drag" >
                    <div className="owl-stage-outer">
                        <Swiper
                            modules={[Autoplay]}
                            spaceBetween={10}
                            slidesPerView={6}
                            loop={true}
                            autoplay={{
                                delay: 4000,
                                disableOnInteraction: false,
                                pauseOnMouseEnter: false,
                            }}
                            speed={1000}
                            breakpoints={{
                                0: {
                                    slidesPerView: 1,
                                },
                                492: {
                                    slidesPerView: 2,
                                },
                                768: {
                                    slidesPerView: 4,
                                },
                                1000: {
                                    slidesPerView: 5,
                                },
                                1200: {
                                    slidesPerView: 6,
                                },
                            }}>

                            {
                                brandsData.map((brand) => (
                                    <SwiperSlide key={brand.id}>
                                        <div className="brand-one__single" >
                                            <div className="brand-one__single-inner">
                                                <Link to="#"><img src={brand?.image} alt='Brand Image' /></Link>
                                            </div>
                                        </div>
                                    </SwiperSlide>
                                ))
                            }
                        </Swiper>
                    </div>
                </div>
            </div>
        </section>

    );
}






